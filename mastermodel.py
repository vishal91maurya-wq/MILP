...
def visualise(SKU_coordinate, sku, dfc, id, title, argv): # function to plot configuration of all SKUs into a single DFC 
    ...
    mypath =mypath = f"./results/{argv[1]}_{argv[2]}_{argv[3]}_{argv[4]}"   # path where to save the image
    #print(mypath)
    #exit(0)
...
# %%
def model(csv, cubic_dfc=0,bound_dfc=0):
    ...
    ampl.eval(
        ...
        param Cubic_dfc binary default 0;
        param bound;
        param M;    
        ...
        s.t. deactivate_empty_copy {k in Copy}:                 # if we don't use this then there are active copies with no skus in it but using it actually increase the number of non-empty skus
            copy_used[k] <= sum{i in SKU} sku_in_copy[i,k];
        """
        )
    ...
    ampl.option['gurobi_options'] = 'timelimit=5 mipgap=0.05 outlev=1'
    ampl.solve()
    ...
    if "infeasible" in str(solve_result).lower():
       ...
       # 1. Prepare SKU Coordinates (Starting points)
        ...
        for k in active_copies:
            ...
            visualise(filtered_coords, filtered_dims, container_dims, gif_filename_id, step_titles, sys.argv)
        ...
        # visualise(currentSKU_Coordinates, currentSKU_Dimensions, currentDFC, f"{argv[1]}_{dfc_instance_name}", title, sys.argv)
...
#model('VmeasureData.csv',1)

# %%
model('VmeasureData.csv',0,1)

# %%
#model('VmeasureData.csv',1,1)
